...
### AD-023: Resolution-Aware Engine Dispatch + Auto-Iteration Rescale

**Date:** 2026-07-06
**Decision:** Engine handoffs are chosen by *pixel spacing*, not fixed zoom
thresholds, and the floatexp kernel owns everything from 1e13 up.

**Problems (user testing):**

1. Vertical streaks at 7e12 on a 1080p preview, Engine STD. The standard
   kernel computes `c = center + offset` in absolute float64; at 7e12/1080p
   the pixel spacing (~4e-16) is below one ulp near |c| ~ 1.5, so adjacent
   pixel columns collapse onto identical coordinates. A fixed 1e13 cutoff
   ignores resolution entirely.
2. Moiré rings at 4.6e17, Engine PT. The float64 PT window (1e13–1e18) runs
   *without* rebasing (float64 + rebasing are incompatible, AD-014), so it
   silently loses precision when the reference orbit passes near zero.
3. Auto max-iter's 60/decade deep slope forced manual bumps (user needed
   20k where it gave 4k at 4.6e17).

**Fixes:** `needs_perturbation(zoom, height)` — deep when pixel spacing
`(3/zoom)/height < 2e-14` (~100 ulps of headroom; 1080p leaves float64 at
~1.4e11, small previews at ~2e12); `DEEP_FXP_LOG10` 18 → 13 (float64 PT
keeps only the narrow safe window below 1e13, where deltas are too large
for near-zero passages to hurt); deep auto-iter slope 60 → 400/decade
(iterations are cheap on GPU when exteriors dominate; orbit is cached).

**Lesson (LL-011):** Any threshold guarding a float64 resolution limit must
account for *resolution*. And when two engines overlap in coverage, prefer
the one that is correct in all regimes over the one that is faster in some.

---

### AD-024: Julia Perturbation Engine

**Date:** 2026-07-06
**Decision:** Add a perturbation path to the Julia engine, dispatched by the
same pixel-spacing rule.

**Problem:** Julia rendering was pure float64 — beyond ~1e12 it dissolved
into per-pixel noise (user screenshot: static mosaic with a black disc).

**Design:** HP reference orbit from `Z_0 = center` (gmpy2/mpmath, cached per
center with sufficiency checks); pixels iterate `d' = 2*Z*d + d²` — no `+dc`
term since c is shared. Julia perturbation is *simpler* than Mandelbrot's:
c is exactly representable in float64, so the standard-iteration
continuation after the reference ends is numerically sound. v1 has no
rebasing/floatexp — practical ceiling ~1e30, far past the ~1e12 failure of
the standard kernel. Deep-zoom targets on J(c) can be generated by inverse
iteration (repelling fixed point pulled back through random branches of
`z → ±sqrt(z−c)`), which is how the tests build 110-digit on-set points.

**Validation:** matches the standard kernel at shallow zoom (median diff
0.0000, classification identical); smooth structured fields at 1e13–1e20
where the old path was static. `tests/test_julia_pt.py`.
